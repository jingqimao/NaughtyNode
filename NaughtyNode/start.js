...
let path_set={
	...
	logs:ROOT_PATH+'logs',
	controller:ROOT_PATH+'webapp/controller',
	service:ROOT_PATH+'webapp/service',
	uilter:ROOT_PATH+'webapp/uilter',
	upload:ROOT_PATH+'webapp/upload',
	view:ROOT_PATH+'webapp/view',
	static:ROOT_PATH+'webapp/view/static',
	cache:ROOT_PATH+'webapp/cache'
};
bestRequire(ROOT_PATH,path_set);
pathUtils.set(path_set);

//读取系统配置
const config = yamljs.parse(fs.readFileSync('./conf/config.yml').toString());

//读取文件类型
const contentTypeJson=require('./conf/contentType.json');
...
const sysArgs={
	...
	cons_path:['./webapp/controller'],//控制器位置
	sers_path:['./webapp/service'],//服务块位置
	tasks_path:['./webapp/task'],//服务块位置
	cons:[],//控制器容器
	sers:[],//服务容器
	sers_list:[],//服务信息列表
	inits:[],//初始化事件容器
...
//添加云服务参数
if(config.node!=undefined&&config.node.client!=undefined&&config.node.client.name!=undefined){
	cloud.client_name=config.node.client.name;
...
//设置log4js日志
let log4js_config=yamljs.parse(fs.readFileSync('./conf/log4js.yml').toString());
...
for(let j in sysArgs.cons_path){
	...
	con_pas.forEach((ele,index)=>{
		let cc=require(ele.replace('.js','')).cons;
		for(let i in cc){
...
//响应文件请求
function callBackFileResponse(url,response,contentType,isStream){
	...
	if(sysArgs.other_api.get[url]!=undefined||sysArgs.other_api.post[url]!=undefined){
		if(request.method === 'POST'){
			...
			request.on('end', function (){
				let args=querystring.parse(data);
			...
			for(let i in fun_args){
				if(fun_args[i]=='$'){
					args[i]={
						...
						outFile:function(_file,_contentType,isStream){
							if(_contentType==undefined)_contentType='application/octet-stream';
							let _res=this.res;
