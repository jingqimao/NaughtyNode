...
app.setPrefix('/backstage_api/content');
...
app.get('/add_material',async function($,mater,data) {
	...
	fs.writeFile(material_dir+file_name,JSON.stringify(data,null,'\t'),function(err){
		if(err){
			$.out(false);
		}else{
...
app.get('/save_material',async function($,mater,index,data) {
	...
	list[index]=mater;
	cfUtils.get('_SYS_Material').set(list).save();
	fs.writeFile(material_dir+mater.file,JSON.stringify(data,null,'\t'),function(err){
		if(err){
			$.out(false);
		}else{
			$.out(true);
...
app.get('/get_material_data',async function($,index) {
	...
	let list=cfUtils.get('_SYS_Material').data;
	$.out(fs.readFileSync(material_dir+list[index].file,'utf8'));
