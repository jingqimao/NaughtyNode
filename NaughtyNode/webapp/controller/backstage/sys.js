...
app.setPrefix('/backstage_api/sys');


//菜单
app.get('/get_menu',async function($) {
	
	let sys_setting_dir=pU.p(':setting');
	let cc='[]';
	if($.session.get('Manager_info')!=undefined){
		if($.session.get('Manager_info').role.indexOf('super_manager')>-1){
			cc=fs.readFileSync(sys_setting_dir+'backstage-menu-super.json','utf8');
		}else{
			cc=fs.readFileSync(sys_setting_dir+'backstage-menu.json','utf8');
		}
	}
...
app.get('/login',async function($,user,pass) {
	...
	for(let i in list){
		if(user===list[i].account&&pass===list[i].password){
			...
			list[i].last_time=commonUtils.now();
			cfUtils.get('_SYS_Manager').set(list).save();
...
app.get('/restart_system',async function($) {
	
	let setting_path=$.sys_root+'setting.yml';
	if(fs.existsSync(setting_path)){
		let setting = yamljs.load(setting_path);
		setting.restart_time=commonUtils.now();
		fs.writeFile(setting_path,yamljs.stringify(setting, 4),function(err){
			if(err){
				$.out(false);
			}else{
				$.out(true);
...
//系统节点
app.get('/get_nodes',async function($) {
