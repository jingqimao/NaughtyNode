...
app.setPrefix('/backstage_api/api');

let sys_api_dir;
let cus_api_dir;
let cus_ser_dir;

const cus_api_temp='/webapp/controller/backstage/cus_api_templete.js';
const cus_ser_temp='/webapp/controller/backstage/cus_ser_templete.js';

app.init(async function($){
	sys_api_dir=pU.p(':controller')+'sys/';
	cus_api_dir=pU.p(':controller')+'custom/';
	cus_ser_dir=pU.p(':service')+'custom/';
});
...
app.get('/new_file',async function($,space,url,type,name) {
	...
	let root=null;
	if(space=='sys_api')root=sys_api_dir;
	if(space=='cus_api')root=cus_api_dir;
	if(space=='cus_ser')root=cus_ser_dir;
	...
	if(root!=null){
		...
		if(type=='file'&&space.indexOf('api')>-1){
			let path=root+url+name;
			if(!fs.existsSync(path)){
				let content=fs.readFileSync($.sys_root+cus_api_temp);
		...
		if(type=='file'&&space.indexOf('ser')>-1){
			let path=root+url+name;
			if(!fs.existsSync(path)){
				let content=fs.readFileSync($.sys_root+cus_ser_temp);
				fs.writeFile(path,content,function(error){
					if(error)$.out(error);
					$.out(true);
				});
